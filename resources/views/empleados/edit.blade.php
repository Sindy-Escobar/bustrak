...
<div class="container">
    ...
    <form action="{{ route('empleados.update', $empleado->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="apellido" value="{{ old('apellido', $empleado->apellido) }}" required>
        </div>

        <div class="form-group">
            <label>DNI</label>
            <input type="text" name="dni" value="{{ old('dni', $empleado->dni) }}" required pattern="\d{13}" title="Debe ser un número de 13 dígitos">
        </div>
        ...
        <div class="form-group">
            ...
            <input type="date" name="fecha_ingreso" value="{{ old('fecha_ingreso', $empleado->fecha_ingreso) }}" required>
        </div>

        <div class="form-group">
            <label>Rol</label>
            <div class="rol-container">
                <div class="rol-card {{ old('rol', $empleado->rol)=='Empleado' ? 'selected' : '' }}" data-value="Empleado">Empleado
                    <input type="radio" name="rol" value="Empleado" style="display:none;" {{ old('rol', $empleado->rol)=='Empleado' ? 'checked' : '' }}>
                </div>
                <div class="rol-card {{ old('rol', $empleado->rol)=='Administrador' ? 'selected' : '' }}" data-value="Administrador">Administrador
                    <input type="radio" name="rol" value="Administrador" style="display:none;" {{ old('rol', $empleado->rol)=='Administrador' ? 'checked' : '' }}>
                </div>
            </div>
            @error('rol')
